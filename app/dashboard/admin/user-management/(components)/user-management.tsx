...
import { CalendarIcon, ETHIcon, SearchNormalIcon } from '@/assets/svgs';
import { Button } from '@/components/shared';
import {
  AllUsersReportResponse,
  Employee,
  getAllUsersReport,
  getEmployees,
} from '@/lib/auth/admin';
...
export default function UserManagement() {
  ...
  const fetchEmployees = useCallback(async () => {
    setLoading(true);
    try {
      const response = await getEmployees(page, perPage);
  ...
  const mappedUsers: ManagementUser[] = employees.map(emp => ({
    id: emp.id,
    username: emp.email?.split('@')[0] || 'N/A',
    name: `${emp.name || 'N/A'} ${emp.surName || ''}`.trim(),
    email: emp.email || 'N/A',
    registrationDate: emp.createdAt ? dayjs(emp.createdAt).format('DD MMM YYYY') : 'N/A',
    assetHoldings: 0, // Not available in API
    affiliates: 0, // Not available in API
    AffiliateStatus: emp.is_rewards_enabled ? 'Yes' : 'No',
    status: emp.hasBeenBlocked ? 'Blocked' : emp.emailVerified ? 'Active' : 'Inactive',
    kycStatus: 'Need Information', // Not available in API
    lastLogin: emp.lastLoginAt ? dayjs(emp.lastLoginAt).format('DD MMM YYYY') : 'N/A',
    avatarUrl: '',
    country: emp.countryCode || 'N/A',
    phone: emp.mobile || 'N/A',
    ip: emp.lastLoginIp || 'N/A',
    referral: emp.referredBy || 'N/A',
    is_demo: emp.is_demo,
    is_admin: emp.is_admin,
    is_rewards_enabled: emp.is_rewards_enabled,
  }));
  ...
  const columns: Column<ManagementUser>[] = [
    ...
    },
    {
      ...
      sortable: true,
      renderCell: u => <span>{format(u.assetHoldings)}</span>,
    },
    {
      key: 'affiliates',
    ...
    },
    {
      ...
      renderCell: u => (
        <span className={u.kycStatus === 'Approved' ? 'text-brand-mint' : 'text-brand-gold/80'}>
          {u.kycStatus}
        </span>
      ),
    },
      ...
      renderCell: u => (
        <div className="flex flex-col items-center gap-2">
          <Button
            ...
            className="w-full !px-3 !py-1 text-[10px] capitalize"
          />
  ...
  return (
    <div className="max-w-full space-y-6 overflow-hidden">
        ...
        <div className="flex items-center gap-2">
          <div className="flex flex-col gap-2">
            ...
            <Button
              ...
              variant="primary"
              className="max-w-max !px-3 !py-2 text-sm capitalize"
        ...
        <div className="flex items-center gap-2 w-full justify-end">
          <div className="relative w-full max-w-md">
            <input
              ...
              className="xl:min-w-96 w-full rounded-full bg-light py-3 pl-4 pr-10 text-xs text-white placeholder-brand-gold focus:outline-none focus:ring-2 focus:ring-brand-gold"
            />
            <SearchNormalIcon className="absolute right-3 top-1/2 -translate-y-1/2 text-xl text-white" />
          </div>

          {/* Date picker */}
          <div ref={dateRef} className="relative z-20 overflow-visible">
            <button
              onClick={toggleCalendar}
              className="flex shrink-0 items-center justify-between gap-5 rounded-full border border-brand-gold px-4 py-2 text-sm text-brand-gold transition-all duration-300 hover:bg-dark"
            >
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-2">
        <div className="rounded-xl border border-transparent bg-light p-6">
          <div className="mb-2 text-sm font-medium text-white/70">Total Deposits</div>
          {reportLoading ? (
            <div className="flex items-center justify-start">
              <CgSpinner className="size-6 h-6 w-6 shrink-0 animate-spin text-brand-gold" />
            </div>
          ) : reportData?.totalDepositeAmounts !== undefined &&
            reportData?.totalDepositeAmountsInUSD !== undefined ? (
            <div className="flex items-center justify-start gap-2 text-2xl text-brand-gold">
            ...
            <div className="text-2xl font-bold text-brand-gold">N/A</div>
          )}
        </div>
