...
import { CalendarIcon, GlobalSearch, MapIcon, ShareIcon } from '@/assets/svgs';
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between p-4">
        ...
        <div className="flex gap-3">
          <Button
            title="Suspend"
            variant="primary"
            className="!px-4 !py-2 text-[10px] capitalize"
            onClick={onAction}
          />
          <Button
            title="Unblock"
      ...
      <div className="flex w-full items-center justify-between p-4 text-sm">
        <InfoItem
          icon={<HiOutlineUser className="size-5 h-5 w-5 shrink-0 text-white" />}
          label="Name"
          value={user.name}
        />
        <InfoItem
          icon={<HiOutlineMail className="size-5 h-5 w-5 shrink-0 text-white" />}
          label="Email"
          value={user.email}
        />
        <InfoItem
          icon={<HiOutlineGlobeAlt className="size-5 h-5 w-5 shrink-0 text-white" />}
          label="Country"
          value={user.country}
        />
        <InfoItem
          icon={<HiOutlinePhone className="size-5 h-5 w-5 shrink-0 text-white" />}
          label="Phone Number"
          value={user.phone}
        />
      </div>
...
function InfoItem({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center gap-2">
      ...
      <div>
        <p className="text-xs text-brand-gold">{label}</p>
        <p>{value}</p>
      </div>
    </div>
  );
    ...
    <div className=" text-sm">
      <div className="flex items-center justify-between  px-4 py-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-brand-gold">Date</p>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-brand-gold">IP</p>
            <p>{user.ip}</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-brand-gold">Location</p>
            <p>{user.country}</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between px-4 py-6">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <p className="text-xs text-brand-gold">Referral Username</p>
            <p>{user.referral}</p>
          </div>
        </div>

        <Button title="View User" variant="outlineBlue" className="!px-4 !py-1 capitalize" />
      </div>
    </div>
  );
}
...
export function SigillumHoldingTable() {
  ...
  const assetColumns: Column<AssetRow>[] = [
    ...
    { key: 'price', header: 'Price', accessor: 'price' },
    {
      ...
      header: 'Status',
      renderCell: row => {
        const cls = row.status === 'Success' ? 'text-brand-mint' : 'text-brand-red';
    ...
    { key: 'tx', header: 'Transaction ref', accessor: 'tx' },
    {
      ...
      header: '',
      renderCell: () => (
        <Button title="Manage Sigillum" variant="outlineBlue" className="!px-4 !py-1 capitalize" />
      ),
    },
  ];
  ...
  const cols: Column<TxRow>[] = [
    ...
    { key: 'category', header: 'Category', accessor: 'category' },
    {
      ...
      renderCell: row => {
        ...
        else if (row.status === 'Pending') cls = 'text-brand-gold/80';
  ...
  const cols: Column<EarningsRow>[] = [
    ...
    { key: 'category', header: 'Category', accessor: 'category' },
    {
      ...
      renderCell: row => {
        ...
        else if (row.status === 'Assigned') cls = 'text-brand-gold';
        else cls = 'text-brand-red';
        return <span className={cls}>{row.status}</span>;
      },
    },
  ...
  return (
    <div className="flex gap-6 text-sm ">
      <div className="flex max-w-fit flex-col justify-between px-4 py-6">
        <InfoItem icon={<CalendarIcon />} label="Request Date" value={user.registrationDate} />
        <div className="flex gap-4">
          <Button title="Approve" variant="primary" className="!px-4 !py-1 capitalize" />
          <Button title="Reject" variant="danger" className="!px-4 !py-1 capitalize" />
        </div>
      </div>
