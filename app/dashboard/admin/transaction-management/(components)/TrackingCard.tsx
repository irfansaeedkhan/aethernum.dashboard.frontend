...
const TrackingCard: React.FC<TrackingCardProps> = ({
  ...
  return (
    <div
      className={`flex items-baseline rounded-xl border border-transparent bg-light p-6 text-xs ${className}`}
    >
      <div className="w-full">
        <div className="mb-2">
          <div className="grid grid-cols-1 sm:grid-cols-3">
            <button className="text-gradient text-base">Track daily</button>
            <button className="text-gradient ml-1 pb-1 text-base">Track Weekly</button>
            <button className="text-gradient ml-2 pb-1 text-base">Track Monthly</button>
          </div>
        </div>
          ...
          {transactionStats.map((item, index) => (
            <div key={index} className="space-y-4 rounded-lg bg-light text-sm">
              <div className="p-4">
                ...
                <div className="text-xs font-semibold text-[#00FF94]">{item.deposits}</div>
              </div>
              <div className="p-4">
