        ...
        <div className="p-6">
          ...
          <div className="mb-6 rounded-lg bg-light p-6">
            <div className="mb-4 flex gap-14 border-b border-[#0E1F30] p-4  pb-4">
              <div>
                <p className="mb-1 text-sm text-brand-gold">Event Date</p>
                <p className="text-sm">{eventData?.eventDate || '12 JAN 2024'}</p>
              </div>
              <div>
                <p className="mb-1 text-sm text-brand-gold">Event Type</p>
                <p className="text-sm">{eventData?.eventType || 'Login'}</p>
              </div>
              <div className="">
                <p className="mb-1 text-sm text-brand-gold">Event Description</p>
                <p className="text-sm">
            ...
            <div className="mb-4  flex gap-14 border-b border-[#0E1F30] p-4">
              <div>
                <p className="mb-1 text-sm text-brand-gold">IP</p>
                <p className="text-sm">{eventData?.ip || '128.1.123.970'}</p>
              </div>
              <div>
                <p className="mb-1 text-sm text-brand-gold">Severity Level</p>
                <p
                  className={`text-sm ${
            ...
            <div className="flex justify-between p-4">
              <div>
                <p className="mb-1 text-sm text-brand-gold">UserID</p>
                <p className="text-sm">{eventData?.userId || '12175688'}</p>
              </div>
              <div>
                <p className="mb-1 text-sm text-brand-gold">Username</p>
                <p className="text-sm">{eventData?.username || 'John Doe'}</p>
              </div>
              <div>
                <p className="mb-1 text-sm text-brand-gold">Name and Surname</p>
                <p className="text-sm">{eventData?.nameAndSurname || 'John Doe'}</p>
              </div>
              <div className="flex items-center gap-6">
                <div>
                  <p className="mb-1 text-sm text-brand-gold">Email</p>
          ...
          <div className="mb-6 rounded-lg bg-light p-4 text-sm">
            ...
            <div className="max-h-40 space-y-4 overflow-y-auto pr-2 scrollbar-thin scrollbar-track-dark scrollbar-thumb-brand-gold">
              {[
                'Logins / IP',
                ...
                <div key={label} className="flex border-b border-dark pb-2">
                  ...
                  <div className="w-3/4 text-brand-gold">
          ...
          <div className="mt-6 flex justify-end space-x-4">
            <Button
              title="  Flag Event"
              variant="primary"
              className="rounded-full !px-4 !py-2 text-[10px] capitalize"
              onClick={() => onClose()}
            />
            <Button
