...
const SigillumTable = () => {
  ...
  const getActionButton = (status: string) => {
    if (status === 'Suspend') {
      return <Button title="Suspend" variant="danger" className="rounded-full text-xxs" />;
    } else {
      return <Button title="Active" variant="primary" className="rounded-full text-xxs" />;
    }
  };
    ...
    <div className="text-white">
      ...
      <div className="flex items-center justify-end gap-2 rounded-bl-2xl rounded-br-2xl bg-light p-4">
        <button
          ...
          className="flex items-center justify-center rounded-full p-2 text-white hover:text-brand-gold disabled:opacity-50"
        >
          <CircleChevronLeft />
        </button>
        ...
        {Array.from({ length: totalPages }, (_, i) => (
          <button
            ...
            className={`
              ...
              ${currentPage === i + 1 ? 'border border-[#00FF94] text-[#00FF94]' : 'text-gray-300'}
            `}
          >
            {i + 1}
          </button>
