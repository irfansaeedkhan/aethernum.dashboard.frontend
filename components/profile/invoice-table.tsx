...
export const InvoiceTable = () => {
  ...
  const getInvoicesDetails = useCallback(async () => {
    setLoading(true);
    try {
      const res: InvoicesListResponse = await getInvoicesList();
      setInvoicesData(res?.Invoices ?? []);
      setTotalPages(res?.Number_of_Pages ?? 1);
    } catch (e) {
      console.error(e);
    } finally {
      setLoading(false);
  ...
  const filteredTransactions =
    ...
    invoicesData.filter(transaction => {
      ...
      return transaction.date.toLowerCase().includes(selectedYear.replace('_', '-').slice(0, 4));
    });
  ...
  const computedTotalPages = Math.max(
    1,
    Math.ceil((filteredTransactions?.length || 0) / pageSize) || totalPages
  );

  const handleDownloadPDF = async (invoice_id: string) => {
    try {
      const response = await axiosAPIBlockchain.get(`/invoices/${invoice_id}/pdf`, {
        responseType: 'blob',
      });
      ...
      toast.error('Invoice download is demo-only right now.');
    }
  };
              ...
              <td colSpan={6}>
                <div className="mx-auto flex w-full max-w-[15rem] items-center justify-center gap-4 py-4">
                  ...
                  <button
                    ...
                    disabled={page >= computedTotalPages || computedTotalPages <= 1}
                    className="text-blue-shade-1 flex w-full items-center justify-center gap-2 rounded-lg  border-[1px] border-[#1414141F] bg-white px-4 py-2 text-sm disabled:opacity-50"
                  >
