...
export const ReferralTableMobile: React.FC<Props> = ({
  ...
  // const totalPages = Math.ceil(affiliatesList.total / affiliatesList.limit);
  ...
  return (
    <div ref={tableSectionRef} className="flex w-full flex-col rounded-xl bg-primary">
      <div className="mb-4 flex h-16 items-center justify-between gap-5 rounded-xl bg-light px-6 py-4">
      ...
      {affiliatesList && affiliatesList?.length > 0 && isLoading === 'resolved' ? (
        <>
          {affiliatesList.map((item, index) => (
            <div
              key={index}
              className="border-black-shade-1/[0.12] mb-4 flex w-full flex-col gap-3 rounded-xl border-none bg-light p-6 last:mb-0"
            >
