...
import { ExclaimationIcon, DollarCoin } from '@/assets/svgs';
...
export const InvoiceTableMobile: React.FC = () => {
  ...
  const filteredTransactions =
    ...
    invoicesData.filter(transaction => {
      ...
      return transaction.date.toLowerCase() === selectedYear;
    });
  // pdf
  ...
  const handleDownloadPDF = async (invoice_id: string) => {
    try {
      const response = await axiosAPIBlockchain.get(`/invoices/${invoice_id}/pdf`, {
        responseType: 'blob', // Set the response type to blob for binary data (PDF file)
      });
  ...
  return (
    <div className=" flex w-full flex-col rounded-xl text-white md:hidden">
            ...
            >
              ...
              <div className={mainDiv}>
                ...
                <button onClick={() => handleDownloadPDF(invoice?.id)}>
                  <RiDownload2Fill className="size-5 h-5 w-5 shrink-0 text-white" />
                </button>
              </div>
            </div>
          ))
      ...
      {/* 
      {invoicesData && invoicesData?.Number_of_Pages > 0 && (
        <div className="mx-auto flex w-full max-w-[90%] items-center justify-center gap-4 py-4">
          ...
          <button
            ...
            disabled={page === invoicesData?.Number_of_Pages}
            className="flex w-full items-center justify-center gap-2 rounded-lg border-[1px]  border-[#1414141F] px-4 py-2 text-sm disabled:opacity-50"
          >
