...
export const YearDropdown: React.FC<DropdownProps> = ({
  ...
  return (
    <div className="relative w-full max-w-[120px] flex-grow" ref={ref}>
      ...
      {isOpen && (
        <ul className="absolute z-50 mt-2 flex max-h-[220px] w-full flex-col items-center justify-center overflow-y-auto rounded-md  border bg-dark text-white shadow-lg hover:text-brand-gold">
          {options.map(option => (
            <li
              key={option.value}
              className={clsx(
                'word-break w-full cursor-pointer px-2  py-1 text-xs text-white hover:bg-light hover:text-brand-gold',
