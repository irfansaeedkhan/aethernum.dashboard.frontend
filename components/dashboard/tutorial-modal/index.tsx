...
export const TutorialModal: React.FC<TutorialModalProps> = ({ open, onClose, slides }) => {
  ...
  return (
    <ModalContainer
      ...
      isOpen={open}
      onClose={onClose}
      modalContentClassName="h-auto rounded-xl max-w-[600px] w-full"
      ...
      <div className="flex w-full flex-col">
        ...
        <div className="mb-10 flex flex-col items-center gap-10">
          ...
          <div className="flex flex-col items-center gap-1">
            {/* Title */}
            <h2 className="text-center font-nexablack text-xl font-black uppercase leading-tight text-brand-gold sm:text-2xl">
        ...
        <div className="flex items-center justify-between gap-3 md:gap-4">
          ...
          {isFirstSlide && (
            <div className="flex-1  md:w-[40%] md:flex-initial">
              <Button
                className="w-full md:!w-[120px] md:!px-6 md:!py-2"
                title="Skip"
                onClick={handleSkip}
              />
            </div>
          ...
          {/* Center - Skip text link for mobile (middle slides) */}
          {!isFirstSlide && !isLastSlide && (
            <div className="flex flex-1 items-center justify-center md:hidden">
              <button
                onClick={handleSkip}
                className="px-3 text-sm font-medium uppercase text-brand-gold transition-colors hover:text-brand-gold/80"
              >
                Skip
              </button>
            </div>
          ...
          <div className="flex flex-1 items-center justify-end md:w-[40%] md:flex-initial">
            {/* Skip Button - Desktop only, middle slides */}
            {!isFirstSlide && !isLastSlide && (
              <div className="hidden md:block">
                <button
                  className="px-3 text-[10px] font-medium uppercase text-brand-gold transition-colors hover:text-brand-gold/80"
                  onClick={handleSkip}
                >
                  Skip
                </button>
