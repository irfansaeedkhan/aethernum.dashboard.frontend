...
export const LeaderboardTableMobile: React.FC<Props> = ({
  ...
  return (
    <div className="flex w-full flex-col rounded-xl border-gray bg-light">
      <div className="mb-4 flex h-16 items-center justify-between gap-5 rounded-t-xl bg-light px-6 py-4">
        <div className="flex items-center gap-4">
          <span className="text-lg font-medium leading-7 text-white">Leaderboard</span>
          <span className="flex max-w-max items-center justify-center rounded-xl border border-brand-gold bg-primary px-2 py-1 text-xs font-medium leading-[10px] text-brand-gold">
      ...
      {leaderboardList?.LeaderBoard?.length > 0 && (
        <>
          {leaderboardList?.LeaderBoard.map((item, index) => (
            <div
              key={index}
              className="mb-4 flex w-full flex-col gap-3 rounded-xl border-none border-gray bg-light p-6 last:mb-0"
            >
          ...
          <div className="mx-auto flex w-full max-w-[15rem] items-center justify-center gap-4 py-4">
            ...
            <button
              ...
              disabled={page === leaderboardList?.Number_of_Pages}
              className="flex w-full items-center justify-center gap-2 rounded-lg border-[1px]  border-[#1414141F] bg-light px-4 py-2 text-sm text-white disabled:opacity-50"
            >
