...
export const RewardListTableMobile: React.FC<Props> = ({
  ...
  const pageSize = 5;
  const pageData = rewardListData.slice((page - 1) * pageSize, page * pageSize);
  const computedTotalPages = Math.max(
    1,
    totalPages || Math.ceil(rewardListData.length / pageSize)
  );
          ...
          >
            ...
            <div className={mainDiv}>
              ...
              <p className={p}>
                ...
                {item?.amount != null ? Number(item.amount).toFixed(2) : 'N/A'} US$
              </p>
            </div>
            <div className={mainDiv}>
